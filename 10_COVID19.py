...
#           업로드 후 : 파일 객체가 있다는 뜻이므로 True이고 분석 실행 가능
if uploaded_confirmed and uploaded_deaths and uploaded_recovered : # 세 개의 객체 모두 True일 때,
    ...
    def get_korea_data(df, value_name):
        ...
        # 열 이름 지정 
        korea_series.columns = ['date', value_name] # 그냥 01234가 아니라 의미있는 이름으로 바꾼다

        # 날짜 문자열 --> datatime 형 변환
    ...
    #   .diff() --> 그리고 NaN는 .fillna(0) --> 그리고 diff()하면 float으로 나오므로 .astype(int)
    ...
    with tab1:
        ...
        # 누적 데이터용 매핑 사전 # ~~~ 때문에 한글을 영어로 바꿔주는거??
        label_map = {
            '확진자' : 'confirmed',
            '사망자' : 'deaths',
            '회복자' : 'recovered'
        ...
        selected_labels = st.multiselect(
            label='표시할 항목을 선택하세요',
            options=list(label_map.keys()), # key만 추출해서 list로 형 변환하겠다
            default=['확진자', '회복자']
        )

        # 13. 하나 이상 선택을 했다면, 그래프를 출력하겠다 
        if selected_labels :
            ...
            fig = px.line(
                ...
                x='date', y=selected_cols, markers=True, 
...
# clone하고 uv sync싱크도 해주면 자동으로 uv가 맞춰서 저장돼요!




# 타입 힌트에서
# Any | None : 이 변수에는 "어떤 타입이 들어오든 상관없"고, 혹은 아예 "값이 비어있어도 된다"는 뜻
# = None : 아무것도 넣지 않으면 자동으로 None을 집어넣겠다

# git bash에 직접 입력하는 것과 terminal에 입력하는거 ㄴ같음?
