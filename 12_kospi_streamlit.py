...
fig_top10 = go.Figure(go.Bar(
    x=top10['Marcap'] / 1e12,
    y=top10['Name'],
    orientation='h', # 수평(가로) 막대그래프
    text=top10['Marcap'] / 1e12,
    texttemplate='%{text:.1f}조', # 소수 첫째자리까지 + '조'라는 단위 붙이기
...
# 종목명을 정규화 함수 적용
selected_stocks = [normalize_str(s) for s in selected_stocks]  # 2 함수를 적용
# 이렇게 for문으로 한줄로 만들고 리스트로 반환하는 걸 
# 리스트 컴프리헨션(리스트 내포)  --> [최종결과 for 변수명 in 리스트명]
...
def get_stock_data(code, start, end):
    ...
    반환값(Returns) :
    DataFrame 형태로 반환
    이때 조회 실패 또는 데이터가 없을 시 None을 반환
    """
    try:
        ...
        if df.empty:
            return None
        return df
    except Exception as e:  # Exception:except들의 엄마
...
for i, code in enumerate(codes): 
    # codes는 리스트 [] 파이썬에서 리스트, 튜플, 문자열 같은 순서가 있는 자료형(iterable)을 루프(for문)로 돌릴 때, 현재 아이템이 몇 번째인지(인덱스)를 동시에 알고 싶을 때 사용
    df = get_stock_data(
        code,
...
with tab1:
    if len(codes) == 1: # 종목을 하나만 선택했다면
        ...
        if df is not None:
            # st.line_char 위젯 : 스트림릿 기본 내장 라인 차트
